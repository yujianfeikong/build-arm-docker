...
The `kkFileView` example now preserves the upstream release tarball layout under `/opt/kkFileView-5.0.0`, matching the official image instead of flattening the archive into a custom directory.

The bundled startup command exports two `kkFileView` runtime variables before Java starts:

- `KKFILEVIEW_BIN_FOLDER=/opt/kkFileView-5.0.0/bin`, so the app resolves its live config file from the packaged `config/` directory instead of the source-tree fallback path `/opt/server/src/main/config`
